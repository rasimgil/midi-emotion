...
def generate(
    ...
    if varying_condition is not None:
        ...
        if conditioning == "none":
            ...
            discrete_conditions_tensor = [
                [maps["tuple2idx"][symbol] for symbol in condition_sample]
                for condition_sample in discrete_conditions
            ]
            discrete_conditions_tensor = (
                torch.LongTensor(discrete_conditions_tensor).t().to(device)
            )
    ...
    exclude_symbols = [
        symbol for symbol in maps["tuple2idx"].keys() if symbol[0] == "<"
    ]
    ...
    with torch.no_grad():
        ...
        while i < gen_len:
            ...
            if varying_condition is not None:
                ...
                conditions_tensor = torch.cat(
                    [valences[:, None], arousals[:, None]], dim=-1
                )
            ...
            if penalty_coeff > 0:
                ...
                temp_multiplier = torch.maximum(
                    torch.zeros_like(repeat_counts_array, device=device),
        ...
        for i in range(1):
            ...
            if n_instruments >= min_n_instruments:
                ...
                if conditioning == "none":
                    ...
                    redo_discrete_conditions = None
                    redo_continuous_conditions.append(
                        continuous_conditions[i, :].tolist()
                    )
                    redo_primers = primers
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_dir", type=str, help="Directory with model", required=True
    )
    ...
    parser.add_argument(
        "--min_n_instruments", type=int, help="Minimum number of instruments", default=1
    )
    parser.add_argument(
        "--valence",
        type=float,
        help="Conditioning valence value",
        default=[None],
        nargs="+",
    )
    parser.add_argument(
        "--arousal",
        type=float,
        help="Conditioning arousal value",
        default=[None],
        nargs="+",
    )
    ...
    assert len(args.valence) == len(
        args.arousal
    ), "Lengths of valence and arousal must be equal"
    assert (args.conditioning == "none") == (
        args.valence == [None] or args.arousal == [None]
    ...
    if n_emotion_bins % 2 == 0:
        bin_ids = list(range(-n_emotion_bins // 2, 0)) + list(
            range(1, n_emotion_bins // 2 + 1)
        )
    else:
    ...
    if os.path.exists(model_fp):
        ...
        model.load_state_dict(
            torch.load(model_fp.replace("best_", ""), map_location=device)
        )
    else:
    ...
    if args.conditioning == "discrete_token":
        ...
        for condition in conditions:
            ...
            valence_symbol = valence_symbols[
                np.searchsorted(emotion_bins, valence_val, side="right") - 1
            ]
            arousal_symbol = arousal_symbols[
                np.searchsorted(emotion_bins, arousal_val, side="right") - 1
            ]
